...
//     const { fetchUserAddToCart, cartItems } = useContext(Context); // ✅ Get cart state

//     const handleAddToCart = async (e, product) => {
//         e?.stopPropagation();
//         e?.preventDefault();

//         // ✅ Prevent duplicate API calls if item is already in cart
//         if (cartItems.some(item => item.productId === product._id)) {
//             toast.error("Selected item is already in cart");
//             return;
//         }

//         const response = await addToCart(e, product._id, cartItems, fetchUserAddToCart);

//         if (response?.success) {
//             fetchUserAddToCart(); // ✅ Update cart state
//         }
//     };
...
//                 ) : (
//                     data.map((product) => (
//                         <Link to={"product/" + product?._id} key={product._id} className="w-full min-w-[280px] md:min-w-[320px] max-w-[280px] md:max-w-[320px] bg-white rounded-sm shadow flex">
//                             <div className="bg-slate-200 h-full p-4 min-w-[120px] md:min-w-[145px]">
//                                 <img src={product.productImage[0]} className="object-scale-down h-full hover:scale-110 transition-all" />
//                             </div>
//                             <div className="p-4 grid">
...
//                                     <p className="text-red-600 font-medium">{displayINRCurrency(product?.sellingPrice)}</p>
//                                 </div>
//                                 <button className="text-sm bg-red-600 hover:bg-red-700 text-white px-3 py-0.5 rounded-full" onClick={(e) => handleAddToCart(e, product)}>
//                                     Add to Cart
//                                 </button>
//                             </div>
//                         </Link>
